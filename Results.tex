...
the two was performed (as described in \Cref{alg_IterativeRefinement})
...
In \Cref{fig_RefineOnlyA} it can be seen that only three nodes were
...
\Cref{fig_RefineOnlyB} then $92$ nodes are inserted. Again, the nodes
are inserted very near to local minima/maxima and additionally near saddle
points. If $tol_{RD}$ is halved again to $0.125$, \Cref{fig_RefineOnlyC}, no
more nodes are inserted. This is due to the very poor quality triangles.
The sliver triangles that can be seen in
\Cref{fig_RefineOnlyB,fig_RefineOnlyC} represent nearly planar section
...
\subsection{Node Smoothing Effect}
In order to show the effect of node smoothing, the resultant mesh shown
in \Cref{fig_RefineOnlyB} is used as input and the same value for
$tol_{RD} = 0.25$ is used with \Cref{alg_NodeSmoothing}.
...
\begin{figure}[h!]
  ...
  \subfloat[$tol_{RD}=0.25$, no smoothing]{\label{fig_ComboA}\includegraphics[width=50mm]{Figures/RefineOnly025.png}}
  \subfloat[$tol_{RD}=0.25$, smoothing post]{\label{fig_ComboB}\includegraphics[width=50mm]{Figures/NodeSmooth025.png}}
  \subfloat[$tol_{RD}=0.25$, smoothing during]{\label{fig_ComboC}\includegraphics[width=50mm]{Figures/Reconnect025.png}}

In \Cref{fig_ComboC} the marked difference in mesh quality and mesh
...
(\Cref{fig_ComboA}), $713$ nodes were inserted. The surface area of the
